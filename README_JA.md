...
3.3.2 には、以下に記載するページ送信の明示的な制御、サービス URL の検証、修正版 PDF.js、最小権限のアプリ境界が含まれています。現在のソースを自分で実行する場合は、Node.js 22.13+、Rust、および安定した macOS コード署名 ID（Apple Development または長期利用する自己署名 ID）を使用してください。
...
ダウンロード版はプロジェクトの安定したローカル ID で署名されていますが、Apple の公証は受けていません。初回起動時は、Control キーを押しながら Satori をクリックし、**開く**を選んでもう一度確認してください。
