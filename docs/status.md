...
- Frontend: 81 Node tests cover reader replacement, late zoom/layout callbacks, restored-page rendering, updater success/failure, known and unknown totals, repeated actions, cancellation/retry, persistence-checkpoint failure, release metadata/checksum validation, and platform installation semantics; strict TypeScript checking and the Vite production build pass. PDF.js remains a separate reader-only chunk; the earlier dependency-audit result was not refreshed in this session.
...
### 2026-09-08 · Reader lifecycle and release source consistency

- Pending resize, zoom, spread-layout and render work remains attached to its original reader. Replacing or clearing a reader prevents its later callbacks from changing another book's page or preferences, and releases late canvases. Layout retains the previous page map while awaiting page sizes; opening restores the target scroll position before rendering nearby pages.
- Nine added regressions execute the actual reader and application action functions with controlled PDF/DOM adapters. Eight failed against the previous implementation; all nine now pass. The full 81 frontend tests, TypeScript/Vite build, 43 Rust tests, formatting, release check/Clippy and `dev-live` check pass. Independent review also passed the nine regressions.
...
- Source version remains 3.4.5. No personal documents or learning state were accessed. Browser QA encountered the task execution environment's connection restriction; real PDF.js/WebView gestures and installed native interaction remain unverified for this change.
