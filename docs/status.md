...
- Scanned visual continuity: short page bridges may use Qwen OCR for accuracy and carry the needed original page images when a PDF has no reliable text layer or OCR misses—even when the whole short bridge has no OCR text; three-page reconstruction bridges keep all three, while chapter routes stay at most two representative images.
...
- Figure-aware PDF evidence: when a current page or a short page bridge depends on a numbered figure/table, Satori now sends the corresponding page images alongside the text layer; a figure referenced at the previous page's tail can be picked up from the next page, while long chapter maps remain text-only for speed.
