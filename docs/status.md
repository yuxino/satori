...
- Brand assets: the selected F cat-ear-hood character replaces the previous icon. The default export preserves the original square ivory background and is fully opaque. The unchanged original is tracked as `src-tauri/icons/source.png`; PNG, ICNS, and nine-size ICO exports are documented in [Brand assets](brand-assets.md). These source assets do not change the existing 3.4.5 release packages.
