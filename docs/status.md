...
- **首页阅读凭证视觉（2026-08-23）**：首页从白底编辑文档改为放在深色石墨桌面上的整张收藏级 `READING PASS`。票面使用裁切角、双线内框、微缩防伪纹、票号、侧边齿孔与暖金编号；当前书是带左右打孔和副券虚线的主票联，学习格子、书架和最近理解继续保留在同一票面。顶部“PDF 学习书房”说明已移除，“AI 服务”文字入口改为带 `aria-label` / tooltip 的齿轮设置按钮。已用真实 `.app` 检查默认窗口和设置入口，打开首页与设置均无 SecurityAgent / XARA 日志
