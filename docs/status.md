...
- **首页编辑式修订（2026-08-23）**：移除首页“悟”字方框以及设置页“百 / O / 兼”单字头像，品牌只保留 `satori` 字标，服务类型改为明确文字标签。主书区压缩为更紧凑的编辑式布局，原先空置的右栏改为可直接回看的“上次弄懂”；52 周格子保留但降低视觉重量，书架与最近理解提前进入首屏。全局画布、阅读器和设置页统一为纸白、石墨与墨蓝，不再混用蓝紫雾面或米灰设置背景。已用 `npm run app` 实机检查首页与设置页，二者打开时均未触发 Keychain 授权
...
- **封面与配色修正（2026-08-23）**：首页和阅读工作台从常见 AI 产品的米灰＋淡紫改为冷中性灰、纯白表面和低饱和蓝灰强调。首页不再读取低清 PDF 首屏缩略图；主书与书架按书 ID 从六组编辑色板稳定生成文字排版封面，书名与页数在任意分辨率下保持锐利，不依赖网络或生成式图片
