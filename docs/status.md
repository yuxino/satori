...
- Scanned PDF search: when PDFKit cannot find text in a scanned or mixed book, ⌘F now falls back to cancellable local OCR, jumps to the matching page, and keeps OCR text in a bounded in-memory cache; no scan index is written to disk.
