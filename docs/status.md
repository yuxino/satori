...
- Reading-bar chapter navigator: the current section/chapter is always visible while reading; the TOC menu jumps to any chapter and highlights the current one. Sources prefer the PDF's own outline, falling back to linked course-directory items, and the outline extraction is reused for directory page linking (no duplicate PDF parse)
