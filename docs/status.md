...
- Reading-bar chapter navigator: the current section/chapter is always visible; ⌘T (or the reading-bar button) opens a TOC drawer that lists chapters and subsections with outline-depth indentation, auto-scrolls to and highlights the current position, and jumps on click. Sources prefer the PDF's own outline, falling back to linked course-directory items, and the outline extraction is reused for directory page linking (no duplicate PDF parse)
