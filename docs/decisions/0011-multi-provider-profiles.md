...
本决策取代最初“单一 Qwen 服务”和“不显示自定义地址”的产品限制；百炼仍保留为默认、兼容旧安装的预设。
...
5. 预设服务地址由 Rust 强制使用内置值。自定义地址只允许 HTTPS，或指向 localhost/loopback 的 HTTP；拒绝凭据内嵌、query、fragment 与非 HTTP(S) scheme。设置页在保存前提示，Rust 在每次写盘前重新验证；旧版留下的不安全地址可以加载到设置页修复，但不能保存或发起请求。兼容服务可以明确关闭鉴权，此时后端完全不读取或发送 Key。
