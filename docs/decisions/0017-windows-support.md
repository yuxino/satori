...
4. Windows 工作流允许手动触发，也可由 ADR 0019 的显式 updater 发布工作流复用。它在 GitHub 托管的 `windows-2025` x64 与 `windows-11-arm` ARM64 runner 上分别执行前端测试/构建、锁文件约束下的 Rust 测试/检查/Clippy，以及 unsigned NSIS 打包。所有外部 Actions 使用完整提交 SHA；单独手动运行时只上传短期构建 artifact，不创建或修改 GitHub Release。
...
6. 3.4.4 起，官方 Tauri updater 根据签名 `latest.json` 的 `windows-x86_64` 或 `windows-aarch64` 条目选择当前架构，且必须先验证内置公钥签名。普通 Windows 工作流 artifact 不等同于公开发布；GitHub Releases 页面只在 updater 失败时作为恢复入口。
...
第 10 项的透明背景描述保留 2026-08-30 图标方案的历史。本次选用 F「猫耳兜帽」原画，并按默认方形导出方式保留象牙色背景；当前 ICO 各帧完全不透明。该视觉要求由 [品牌资产说明](../brand-assets.md) 中的当前方案替代，原始图片、九尺寸 ICO 重建方法及验证边界也记录于该文档。名称一致性、安装/卸载检查和其它 Windows 支持边界继续有效。

### 2026-09-08 · 圆形透明图标

用户许可本地透明处理后，当前图标改为从同一认可原画派生的圆形 RGBA 图标，替代上节不透明方形导出。原画 RGB 与分辨率保持不变，仅增加抗锯齿圆形 alpha；ICO 保留九尺寸且各帧四角真正透明。重建与验收事实见[品牌资产说明](../brand-assets.md)。本次仅更新源资产，已发布的 3.4.5 安装包未替换，Windows 名称一致性与安装/卸载验收要求不变。
